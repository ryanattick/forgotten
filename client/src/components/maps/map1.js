  ...
  componentWillMount() {
    if (this.props.currentPuzzleNum >= 10) {
      ...
      this.setState({
        ...
        completedQuests: ['0'].concat(this.state.levelsRemaining.slice(0, this.props.currentPuzzleNum - 1)),
    ...
    if (this.state.puzzles.answers[this.state.currentQuest].toLowerCase() === answer.toLowerCase()) {
      if (this.state.greenclickedQuest) {
        ...
        this.setState({
          ...
          Promise.resolve(Request.post('/mapData', {level: parseInt(this.state.currentQuest)}, (data) => {
          }))
            .then(() => {
