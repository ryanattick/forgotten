...
class Map4 extends React.Component {
  ...
  componentWillMount() {
    if (this.props.currentPuzzleNum >= 40) {
      this.setState({
    ...
    } else if (this.props.currentPuzzleNum !== 30) {
      this.setState({
        currentQuest: this.props.currentPuzzleNum.toString()[1],
        completedQuests: ['0'].concat(this.state.levelsRemaining.slice(0, this.props.currentPuzzleNum - 30)),
        levelsRemaining: this.state.levelsRemaining.slice(this.props.currentPuzzleNum - 30)
      }, () => {
        this.colorPuzzles();
      });
    }
      ...
      if (this.state.greenclickedQuest) {
        ...
        this.setState({
          ...
          if (this.state.currentQuest === '9') {
            Request.post('/mapData', {level: parseInt('3' + this.state.currentQuest) + 1}, (data) => {
              console.log('succeeded');
            });
          } else {
            Request.post('/mapData', {level: parseInt('3' + this.state.currentQuest)}, (data) => {
              console.log('succeeded');
            });
          }
